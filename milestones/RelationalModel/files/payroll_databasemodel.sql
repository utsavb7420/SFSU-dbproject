...
CREATE TABLE IF NOT EXISTS `payrollDB`.`Employee` (
  ...
  `Employee_history_h_id` INT NOT NULL,
  `insurance_i_id` INT NOT NULL,
  `department_d_id` INT NOT NULL,
  PRIMARY KEY (`emp_id`, `insurance_i_id`, `department_d_id`),
  INDEX `fk_Employee_Designation1_idx` (`Designation_d_id` ASC),
  INDEX `fk_Employee_Employee_history1_idx` (`Employee_history_h_id` ASC),
  INDEX `fk_Employee_insurance1_idx` (`insurance_i_id` ASC),
  INDEX `fk_Employee_department1_idx` (`department_d_id` ASC),
  ...
  CONSTRAINT `fk_Employee_Employee_history1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Employee_insurance1`
    FOREIGN KEY (`insurance_i_id`)
    REFERENCES `payrollDB`.`insurance` (`i_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `payrollDB`.`salary_history` (
  ...
  `salary` INT NULL,
  `Employee_emp_id` INT NOT NULL,
  `Employee_insurance_i_id` INT NOT NULL,
  PRIMARY KEY (`s_id`, `Employee_emp_id`, `Employee_insurance_i_id`),
  INDEX `fk_salary_history_Employee1_idx` (`Employee_emp_id` ASC, `Employee_insurance_i_id` ASC),
  CONSTRAINT `fk_salary_history_Employee1`
    FOREIGN KEY (`Employee_emp_id` , `Employee_insurance_i_id`)
    REFERENCES `payrollDB`.`Employee` (`emp_id` , `insurance_i_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `payrollDB`.`attendence` (
  ...
  PRIMARY KEY (`a_id`, `admin_a_id`, `admin_Employee_emp_id`),
  INDEX `fk_attendence_admin1_idx` (`admin_a_id` ASC, `admin_Employee_emp_id` ASC),
  CONSTRAINT `fk_attendence_admin1`
    FOREIGN KEY (`admin_a_id` , `admin_Employee_emp_id`)
    REFERENCES `payrollDB`.`admin` (`a_id` , `Employee_emp_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
