...
import traceback
import pymysql
from payroll.file_utils import write_to_sql_file
...
class DB:
    ...
    def is_database_exists(self, database=None):
        try:
            connection = pymysql.connect(self._host, self._username, self._password)
    ...
    def _execute_query(self, query, values, action=None, backup=True):
        """
        ...
        if backup:
            ...
            values = tuple(values)
            query = query % (values)
            write_to_sql_file('payroll/transactions.sql', query)
    ...
    def run_sql_file(self, sqlfile):
        ...
        try:
            ...
            for query in queries:
                if self._execute_query(query, values=None, backup=False):  # execute each query
                    queries_executed += 1
